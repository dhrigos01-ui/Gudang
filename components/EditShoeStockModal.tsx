...
interface EditShoeStockModalProps {
  item: InventoryItem;
  onClose: () => void;
  onStockUpdated: () => void;
}

export const EditShoeStockModal: React.FC<EditShoeStockModalProps> = ({ item, onClose, onStockUpdated }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      inventoryService.updateShoeStockQuantity(item.id, quantityNum);
      onStockUpdated();
    } catch (err) {
  ...
  return (
    <Modal title="Edit/Penyesuaian Stok Sepatu" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <h4 className="font-semibold text-white">{item.shoeType}</h4>
          <p className="text-sm text-slate-400">Nomor: {item.size}</p>
          <p className="text-sm text-slate-400">Stok Saat Ini: {item.quantity}</p>
        </div>
        <div>
          <label htmlFor="quantity" className="block text-sm font-medium text-slate-300">Jumlah Stok Baru</label>
          <input 
            type="number" 
            id="quantity" 
            value={quantity} 
