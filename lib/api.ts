...
export const addStock = (shoe: Shoe, quantity: number, warehouse: WarehouseCategory, source: string) => fetch(`${API_BASE_URL}/inventory/shoe`, { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify({ shoe, quantity, warehouse, source, operation: 'add' }) }).then(handleResponse);
export const addLeatherStock = (leatherMasterId: string, quantity: number, supplier: string) => fetch(`${API_BASE_URL}/inventory/leather`, { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify({ leatherMasterId, quantity, supplier, operation: 'add' }) }).then(handleResponse);
...
export const sellStock = (itemId: string, quantityToSell: number, customerName?: string) => fetch(`${API_BASE_URL}/inventory/shoe`, { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify({ itemId, quantity: quantityToSell, customerName, operation: 'sell' }) }).then(handleResponse);
